...
namespace Spriter2UnityDX.Animations
{
	...
	public class AnimationBuilder : UnityEngine.Object
	{
		...
		private ScmlFollower scmlFollower = new ScmlFollower ();
			...
			foreach (var kvPair in GetCurves (timeLine, defaultInfo, parentTimeline)) { //Makes sure that curves are only added for properties 
				...
				case ChangedValues.Sprite:
					...
					if (swapper == null) { //Add a Texture Controller if one doesn't already exist
						...
						swapper.Sprites = new Sprite [0];
						var info = (SpriteInfo)defaultInfo;
						swapper.folder = info.folder;
					}
		...
		private void SetKeys (AnimationCurve curve, TimeLine timeLine, ref Sprite[] sprites, Animation animation)
		{
			foreach (var key in timeLine.keys) { //Create a key for every key on its personal TimeLine
				...
				curve.AddKey (new Keyframe (key.time, GetIndexOrAdd (info.folder, ref sprites, Folders [info.folder] [info.file]), inf, inf));
			...
			curve.AddKey (new Keyframe (animation.length, GetIndexOrAdd (lastInfo.folder, ref sprites, Folders [lastInfo.folder] [lastInfo.file]), inf, inf));
		}

		private int GetIndexOrAdd (int folder, ref Sprite[] sprites, Sprite sprite)
		{
			int index = ArrayUtility.IndexOf (sprites, sprite);//If the array already contains the sprite, return index

			if (index < 0) {
				if (S2USettings.Settings.followScmlSprite) {
					//First time we go through here. Let's copy every sprite in sprites
					if (sprites.Length == 0) {
						sprites = scmlFollower.sprites [folder].ToArray ();
					}
					index = scmlFollower.sprites [folder].FindIndex (delegate(Sprite tempSprite) {
						if (tempSprite != null) {
							return tempSprite.name == sprite.name;
						} else
							return false;
					});

					if (index == -1) {
						Debug.LogErrorFormat ("{0} sprite is missing is the SCMLFollower, index used in TextureController will be 0", sprite.name);
						index = 0;
					}
